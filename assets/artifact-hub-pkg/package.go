...
const (
	meshplayCloudBaseURL    = "https://meshplay.khulnasoft.com"
	meshplayCatalogFilesDir = "catalog"
)
...
func main() {
	...
	log, err := logger.New("meshplayio_package", logger.Options{
...
func fetchCatalogPatterns() ([]byte, error) {
	resp, err := http.Get(fmt.Sprintf("%s/api/catalog/content/pattern", meshplayCloudBaseURL))
	if err != nil {
		return nil, ErrHTTPGetRequest(err, fmt.Sprintf("%s/api/catalog/content/pattern", meshplayCloudBaseURL))
	}
...
func processPattern(pattern CatalogPattern, token string) error {
	...
	catalogFilePath := filepath.Join("..", "..", meshplayCatalogFilesDir, pattern.ID)
...
func getPatternImageURL(pattern CatalogPattern) string {
	...
	if pattern.CatalogData.SnapshotURL == nil {
		imageURL = fmt.Sprintf("https://raw.githubusercontent.com/khulnasoft/meshplay-extensions-packages/master/action-assets/design-assets/%s-light.png,https://raw.githubusercontent.com/khulnasoft/meshplay-extensions-packages/master/action-assets/design-assets/%s-dark.png", pattern.ID, pattern.ID)
	} else {
...
URL: 'https://raw.githubusercontent.com/meshplay/meshplay.github.io/master/%s/%s/%s/design.yml'
downloadLink: %s/design.yml
---`, strings.TrimSpace(string(nameYAML)), version, pattern.UserID, userFullName, userInfo.AvatarURL, patternType, compatibility, pattern.ID, patternImageURL, patternInfo, patternCaveats, patternType, slugify(pattern.Name), pattern.ID, meshplayCatalogFilesDir, pattern.ID, version, pattern.ID)
...
func fetchUserInfo(userID string) (UserInfo, error) {
	resp, err := http.Get(fmt.Sprintf("%s/api/identity/users/profile/%s", meshplayCloudBaseURL, userID))
	if err != nil {
		return UserInfo{}, ErrHTTPGetRequest(err, fmt.Sprintf("%s/api/identity/users/profile/%s", meshplayCloudBaseURL, userID))
	}
	defer resp.Body.Close()
...
func invokeGitHubAction(contentID, assetLocation string, ghAccessToken string) error {
	...
	req, err := http.NewRequest("POST", "https://api.github.com/repos/meshplay/meshplay.github.io/actions/workflows/meshmap.yml/dispatches", bytes.NewBuffer([]byte(payload)))
	if err != nil {
